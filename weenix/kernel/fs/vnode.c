...
special_file_read(vnode_t *file, off_t offset, void *buf, size_t count)
{
        ...
        dbg(DBG_PRINT, "(GRADING2A 1.a)\n");
        bytedev_t *bydev = bytedev_lookup(file->vn_devid);
        dbg(DBG_PRINT, "(GRADING2B)\n");
        return bydev->cd_ops->read(bydev, offset, buf, count);
}
/*
...
special_file_write(vnode_t *file, off_t offset, const void *buf, size_t count)
{
        ...
        bytedev_t *bydev = bytedev_lookup(file->vn_devid);
        dbg(DBG_PRINT, "(GRADING2B)\n");
        return bydev->cd_ops->write(bydev, offset, buf, count);
}
...
special_file_mmap(vnode_t *file, vmarea_t *vma, mmobj_t **ret)
{
        ...
        bytedev_t *device = bytedev_lookup(file->vn_devid);
        dbg(DBG_PRINT, "(GRADING3C 1)\n");
        return device->cd_ops->mmap(file, vma, ret);
}
...
special_file_fillpage(vnode_t *file, off_t offset, void *pagebuf)
{
        ...
        return device->cd_ops->fillpage(file, offset, pagebuf);
}
...
special_file_dirtypage(vnode_t *file, off_t offset)
{
        ...
        bytedev_t *device = bytedev_lookup(file->vn_devid);
        dbg(DBG_PRINT, "(NEWPATH)\n");
        return device->cd_ops->dirtypage(file, offset);
}
...
special_file_cleanpage(vnode_t *file, off_t offset, void *pagebuf)
{
        ...
        bytedev_t *device = bytedev_lookup(file->vn_devid);
        dbg(DBG_PRINT, "(NEWPATH)\n");
        return device->cd_ops->cleanpage(file, offset, pagebuf);
}