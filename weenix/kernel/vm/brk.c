...
int do_brk(void *addr, void **ret)
{
        ...
        // If addr is NULL, should "return" the current break.
        if (addr == NULL)
        {
        ...
        if ((curproc->p_start_brk <= addr) && ((uint32_t)addr <= USER_MEM_HIGH))
        {
                dbg(DBG_PRINT, "(GRADING3A)\n");
                vmmap_t *map = curproc->p_vmmap;
                uint32_t vfn = ADDR_TO_PN(curproc->p_start_brk);
                vmarea_t *vmarea = vmmap_lookup(map, vfn);
                uint32_t start_addr = vmarea->vma_end;
                uint32_t end_addr = ADDR_TO_PN(PAGE_ALIGN_UP(addr));
                if (end_addr > start_addr)
                {
                        dbg(DBG_PRINT, "(GRADING3A)\n");
                        if (!vmmap_is_range_empty(curproc->p_vmmap, start_addr, end_addr - start_addr))
                        {
                                dbg(DBG_PRINT, "(NEWPATH)\n");
                                return -ENOMEM;
                        }
                        vmarea->vma_end = end_addr;
                }
                else if (end_addr < start_addr)
                {
                        dbg(DBG_PRINT, "(GRADING3D 1)\n");
                        vmmap_remove(curproc->p_vmmap, end_addr, start_addr - end_addr);
                }
