...
void handle_pagefault(uintptr_t vaddr, uint32_t cause)
{
        ...
        uint32_t add_to_pn = ADDR_TO_PN(vaddr);
        vmarea_t *fvma = vmmap_lookup(curproc->p_vmmap, add_to_pn);
        if (fvma == NULL)
        {
                dbg(DBG_PRINT, "(GRADING3C 5)\n");
                kthread_exit((void *)EFAULT);
        }
        else if (!(cause & FAULT_WRITE || cause & FAULT_EXEC))
        {
                dbg(DBG_PRINT, "(GRADING3A)\n");
                if (!(fvma->vma_prot & PROT_READ))
                {
        ...
        if (pframe_lookup(fvma->vma_obj, add_to_pn - fvma->vma_start + fvma->vma_off, flag, &respf) != 0)
        {
                dbg(DBG_PRINT, "(GRADING3D 2)\n");
                kthread_exit((void *)EFAULT);
        }
        else
        {
                ...
                dbg(DBG_PRINT, "(GRADING3A)\n");
                uintptr_t addr = (uintptr_t)pt_virt_to_phys((uintptr_t)respf->pf_addr);
                int code = pt_map(curproc->p_pagedir, (uintptr_t)PAGE_ALIGN_DOWN(vaddr), addr, pdFlag, ptFlag);
        }
        dbg(DBG_PRINT, "(GRADING3A)\n");
}