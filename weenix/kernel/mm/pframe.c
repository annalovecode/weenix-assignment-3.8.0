...
int pframe_get(struct mmobj *o, uint32_t pagenum, pframe_t **result)
{
        ...
        pframe_t *r_pframe = NULL;
        int resident = 0;
        while (!resident)
        {
           
                dbg(DBG_PRINT,"(GRADING3A)\n");
                r_pframe = pframe_get_resident(o, pagenum);
                if (r_pframe != NULL)
                {
                     
                        dbg(DBG_PRINT,"(GRADING3A)\n");
                        if (pframe_is_busy(r_pframe))
                        {
                               
                                dbg(DBG_PRINT,"(GRADING3B 7)\n");
                              

                                sched_sleep_on(&r_pframe->pf_waitq);
                        }
                        else
                        {
                ...
                else
                {
                        ...
                        if ((r_pframe = pframe_alloc(o, pagenum)) == NULL)
                        {
                                dbg(DBG_PRINT,"(GRADING3A)\n");
                                
                                return -ENOMEM;
                        }
                        if (pframe_fill(r_pframe) < 0)
                        {
                                dbg(DBG_PRINT,"(GRADING3D 2)\n");
                               
                                return pframe_fill(r_pframe);
                        }
                        resident = 1;
                }
        }
...
void pframe_pin(pframe_t *pf)
{
        ...
        // If the pframe has not yet been pinned, remove this pframe's list link from the allocated list and add it to the pinned list.
...
void pframe_unpin(pframe_t *pf)
{
        ...
        if (pf->pf_pincount == 0)
        {
                dbg(DBG_PRINT,"(GRADING3A)\n");
                list_remove(&pf->pf_link);
                list_insert_before(&alloc_list, &pf->pf_link);
                nallocated += 1;
                npinned -= 1;
        }
        dbg(DBG_PRINT,"(GRADING3A)\n");
