...
sys_read(read_args_t *arg)
{
        ...
        read_args_t kaddr;
        size_t nbytes = sizeof(read_args_t);

        int copy_from_user_return = copy_from_user(&kaddr, arg, nbytes);
        if (copy_from_user_return < 0)
        {
                dbg(DBG_PRINT, "(NEWPATH)\n");
                curthr->kt_errno = -copy_from_user_return;
                return -1;
        }
        kaddr.buf = page_alloc();
        int do_read_return = do_read(kaddr.fd, kaddr.buf, kaddr.nbytes);
        if (do_read_return < 0)
        {
                dbg(DBG_PRINT, "(GRADING3D 1)\n");
                page_free(kaddr.buf);
                curthr->kt_errno = -do_read_return;
                return -1;
        }

        int copy_to_user_return = copy_to_user(arg->buf, kaddr.buf, do_read_return);
        if (copy_to_user_return < 0)
        {
                dbg(DBG_PRINT, "(NEWPATH)\n");
                page_free(kaddr.buf);
                curthr->kt_errno = -copy_to_user_return;
                return -1;
        }
        page_free(kaddr.buf);
        dbg(DBG_PRINT, "(GRADING3C 1)\n");
        return do_read_return;
}
/*
...
sys_write(write_args_t *arg)
{
        ...
        write_args_t kaddr;
        size_t nbytes = sizeof(write_args_t);
        ...
        int copy_from_user_return2 = copy_from_user(kaddr.buf, arg->buf, arg->nbytes);
        if (copy_from_user_return2 < 0)
        {
                dbg(DBG_PRINT, "(NEWPATH)\n");
                page_free(kaddr.buf);
                curthr->kt_errno = -copy_from_user_return2;
                return -1;
        }

        int do_write_return = do_write(kaddr.fd, kaddr.buf, kaddr.nbytes);
        if (do_write_return < 0)
        {
                dbg(DBG_PRINT, "(GRADING3D 1)\n");
                page_free(kaddr.buf);
                curthr->kt_errno = -do_write_return;
                return -1;
        }

        page_free(kaddr.buf);
        dbg(DBG_PRINT, "(GRADING3D 3)\n");
        return do_write_return;
}
...
sys_getdents(getdents_args_t *arg)
{
        ...
        size_t nbytes = sizeof(getdents_args_t);
        ...
        kaddr.dirp = kmalloc(sizeof(dirent_t));

        int ndirents = (int)kaddr.count / sizeof(dirent_t);
        int read_byte = 0;
        int flag = 0;
        int ret = 0;
        ...
        {
                dbg(DBG_PRINT, "(GRADING3D 3)\n");
                int read_byte = do_getdent(kaddr.fd, kaddr.dirp);
                if (read_byte == 0)
                {
                        dbg(DBG_PRINT, "(GRADING3D 3)\n");
                        curthr->kt_errno = -read_byte;
                        break;
                }
                else if (read_byte < 0)
                {
                ...
                int copy_to_user_return = copy_to_user(arg->dirp + i, kaddr.dirp, read_byte);
                if (copy_to_user_return < 0)
                {
                        dbg(DBG_PRINT, "(NEWPATH)\n");
                        kfree(kaddr.dirp);
