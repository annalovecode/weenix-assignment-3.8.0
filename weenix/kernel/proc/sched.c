...
int sched_cancellable_sleep_on(ktqueue_t *q)
{
        ...
        //first check if curthr is cancelled or not. If not:
        if (curthr->kt_cancelled != 1)
        {
                curthr->kt_state = KT_SLEEP_CANCELLABLE;
                // put curthr into wait queue
                ktqueue_enqueue(q, curthr);
                // At same time, CPU is free now, give to the first thread in the run_queue
                sched_switch();
                dbg(DBG_PRINT, "(GRADING1C)\n");
                return curthr->kt_cancelled != 1 ? 0 : -EINTR;
        }
        //If yes, do not put into the queue
        else
        {
                dbg(DBG_PRINT, "(GRADING1C)\n");
                return -EINTR;
...
void sched_cancel(struct kthread *kthr)
{
        ...
        // Cause all threads can only be cancelled in Running state,
        // so if kthr->kt_state is in KT_SLEEP_CANCELLABLE, need to change state: Cancellable Sleep->Runnable->Running->Exited
        // Therefore, remove kthr from wait queue, add to runnable instead.
        kthr->kt_cancelled = 1;
...
void sched_switch(void)
{
        ...
        // refers to Lecture14-Slides2-15(Modified thread_switch)

        context_t *oldContext, *currntContext;
        uint8_t oldIPL;
        oldIPL = intr_getipl(); /* Retreives the current interrupt priority level. */
        intr_setipl(IPL_HIGH);  // masking all interrupts,no more new interrupt

        // no threads on the run queue , then all kernel threads are waiting for an interrupt,
        while (sched_queue_empty(&kt_runq))
        {
                // re-enable interrupts and wait for one to occur in the hopes that a thread gets put on the run queue from the interrupt context.
                intr_disable();
                intr_setipl(IPL_LOW);
                intr_wait(); // Cite: Based on file : interrupt.h
                intr_setipl(IPL_HIGH);
        }
        dbg(DBG_PRINT, "(GRADING1C)\n");
        // Now, we find a runnable thread, switch to new thread

        // intr_setipl(IPL_HIGH);
        oldContext = &curthr->kt_ctx;
        curthr = ktqueue_dequeue(&kt_runq);
        curproc = curthr->kt_proc;
        currntContext = &curthr->kt_ctx;
        context_switch(oldContext, currntContext);
        intr_setipl(oldIPL);
        dbg(DBG_PRINT, "(GRADING1C)\n");
}
