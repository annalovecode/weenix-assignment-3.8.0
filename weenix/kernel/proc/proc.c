...
proc_create(char *name)
{
        ...
        strncpy(pr->p_comm, name, strnlen(name, PROC_NAME_LEN - 1));
        pr->p_comm[PROC_NAME_LEN - 1] = '\0';
        pr->p_pid = _proc_getid();
        KASSERT(PID_IDLE != pr->p_pid || list_empty(&_proc_list));    /* pid can only be PID_IDLE if this is the first process */
                                                                      // dbg(DBG_PRINT, "(GRADING1A 2.a)\n");
        KASSERT(PID_INIT != pr->p_pid || PID_IDLE == curproc->p_pid); /* pid can only be PID_INIT if the running process is the "idle" process */
        dbg(DBG_PRINT, "(GRADING1A 2.a)\n");
        pr->p_pagedir = pt_create_pagedir();

        // parent proc
        pr->p_pproc = curproc;
        // initialize wait queue, thread list and child list
        sched_queue_init(&pr->p_wait);
        list_init(&pr->p_threads);
        list_init(&pr->p_children);

        list_init(&pr->p_list_link);
        list_init(&pr->p_child_link);

        if (NULL != curproc)
        {
                list_insert_tail(&curproc->p_children, &pr->p_child_link);
...
#endif
#ifdef __VM__
        pr->p_vmmap = vmmap_create();
        pr->p_vmmap->vmm_proc = pr;
#endif
        return pr;
}
...
void proc_cleanup(int status)
{
        ...
        // Reparenting any children to the init process
        proc_t *procChildren;
        list_iterate_begin(&curproc->p_children, procChildren, proc_t, p_child_link)
        {
                list_insert_tail(&proc_initproc->p_children, &(procChildren->p_child_link));
                procChildren->p_pproc = proc_initproc;
                dbg(DBG_PRINT, "(GRADING1C)\n");
        }
        list_iterate_end();
#ifdef __VFS__
        ...
        // if (curproc->p_cwd->vn_refcount >= 1 && curproc->p_cwd != NULL)
        // {
        //         vput(curproc->p_cwd);
        //         dbg(DBG_PRINT, "(GRADING2B)\n");
        // }
...
void proc_kill(proc_t *p, int status)
{
        ...
        dbg(DBG_PRINT, "(GRADING1C)\n");
        kthread_cancel(list_tail(&p->p_threads, kthread_t, kt_plink), (void *)status);
}
...
void proc_kill_all()
{
        ...
        {
                if (curproc != p && p->p_pproc != NULL && PID_IDLE != p->p_pproc->p_pid)
                {
                        dbg(DBG_PRINT, "(GRADING1C)\n");
                        proc_kill(p, 0);
                }
        ...
        list_iterate_end();
        if (curproc->p_pproc != NULL && PID_IDLE != curproc->p_pproc->p_pid)
        {
                dbg(DBG_PRINT, "(GRADING1C)\n");
                proc_kill(curproc, 0);
        }
...
pid_t do_waitpid(pid_t pid, int options, int *status)
{
        ...
        {
                if (child_proc->p_pid == pid || pid == -1)
                {
                        while (child_proc->p_state != PROC_DEAD)
                        {
                        ...
                        child_proc->p_pproc = NULL;
                        kthread_t *child_thread = list_tail(&child_proc->p_threads, kthread_t, kt_plink);
