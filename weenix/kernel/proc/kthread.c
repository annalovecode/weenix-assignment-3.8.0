...
kthread_create(struct proc *p, kthread_func_t func, long arg1, void *arg2)
{
        ...
        // thread->kt_ctx =
        thread->kt_kstack = alloc_stack(); /* the kernel stack */
        thread->kt_retval = (void *)0;     /* this thread's return value */
        ...
        thread->kt_state = KT_NO_STATE;    /* this thread's state illegal state*/
        list_link_init(&thread->kt_qlink);
        list_link_init(&thread->kt_plink);
        list_insert_tail(&p->p_threads, &thread->kt_plink); // add thread's proc thread list into p
...
kthread_clone(kthread_t *thr)
{
        ...
        kthread_t *n_thread = (kthread_t *)slab_obj_alloc(kthread_allocator);

        n_thread->kt_kstack = alloc_stack();
        context_setup(&(n_thread->kt_ctx), NULL, 0, NULL, n_thread->kt_kstack, DEFAULT_STACK_SIZE, thr->kt_proc->p_pagedir);

        n_thread->kt_proc = NULL;
        n_thread->kt_state = thr->kt_state;
        n_thread->kt_retval = thr->kt_retval;
        n_thread->kt_errno = thr->kt_errno;
        list_link_init(&n_thread->kt_plink);
        list_link_init(&n_thread->kt_qlink);

        n_thread->kt_cancelled = thr->kt_cancelled;
        n_thread->kt_wchan = thr->kt_wchan;
        KASSERT(KT_RUN == n_thread->kt_state);
        dbg(DBG_PRINT, "(GRADING3A 8.a)\n");
        return n_thread;
}
/*
